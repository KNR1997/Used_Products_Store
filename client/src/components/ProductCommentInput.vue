...
const comment = ref('');
const emit = defineEmits(['post-comment', 'close']);
const props = defineProps(['userReview']);

onMounted(() => {
    if(props.userReview != null) {
        comment.value = props.userReview
    }
})
    ...
    <form @submit.prevent="handleSubmit" method="post">
        <div class="w-full mb-4 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
            <div class="px-4 py-2 bg-white rounded-t-lg dark:bg-gray-800">
                ...
                <textarea id="title" rows="1" class="w-full px-0 text-sm text-gray-900 font-bold bg-white border-0 dark:bg-gray-800 focus:ring-0 dark:text-white dark:placeholder-gray-400" placeholder="Title..." required></textarea>
